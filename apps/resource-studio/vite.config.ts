...
import { parseWav, VOICE_SAMPLE_RATE } from '../../packages/dubbing-core/src/index';
    ...
    configureServer(server) {
      server.middlewares.use('/__dubbing', async (request, response) => {
        ...
        try {
          ...
          if (request.url === '/sync') {
            await execute('bun', ['../../tools/dubbing/dubbing.ts', 'sync', language], { cwd: studio });
          ...
          if (request.url === '/check') {
            await execute('bun', ['../../tools/dubbing/dubbing.ts', 'check', language], { cwd: studio });
          ...
          const root = resolve(repository, 'content', 'dubbing', language);
          ...
          await execute('bun', ['../../tools/dubbing/dubbing.ts', 'organize', language], { cwd: studio });
