  ...
  {:else}
    ...
    {#if font}
      ...
      <section class="font-actions">
        <span class="subtle">Zoom</span>
        {#each [1, 2, 3, 4] as scale (scale)}
          <button type="button" class:active={glyphScale === scale} onclick={() => (glyphScale = scale)}
            >{scale}×</button
          >
        {/each}
      </section>
      ...
      <section class="font-grid" style={`--font-scale: ${glyphScale / 3}`}>
        {#each font.glyphs.slice(visibleStart(), visibleStart() + visibleCount()) as glyph, index (visibleStart() + index)}<article
          ...
          >
            <div class="glyph-preview"><FontGlyph {glyph} scale={glyphScale} /></div>
            {#if glyphScale > 1}<code>#{(visibleStart() + index).toString().padStart(4, '0')}</code><strong
