<script lang="ts">
  import type { SysGlyph } from '@doraemon-monopoly/dubbing-core';
  ...
  $effect(() => {
    ...
    for (let i = 0; i < glyph.pixels.length; i += 1) {
      ...
      image.data[p + 1] = 0;
      image.data[p + 2] = 0;
      image.data[p + 3] = glyph.pixels[i] === 0 ? 255 : 0;
    }
    context.putImageData(image, 0, 0);
  });
</script>

<canvas bind:this={canvas} style:width={`${glyph.width * scale}px`} style:height={`${glyph.height * scale}px`}
></canvas>
