...
export async function encodeIndexedPng(image: IndexedPng, transparent?: number) {
  if (
    ...
    image.alpha.length !== image.pixels.length
  ) {
...
export async function decodeIndexedPng(data: Uint8Array): Promise<IndexedPng> {
  ...
  while (position + 12 <= data.length) {
    ...
    if (crc32(data.subarray(position + 4, position + 8 + length)) !== expectedCrc)
    ...
    if (type === 'IHDR') {
      ...
      width = u32be(payload, 0);
      height = u32be(payload, 4);
      sawHeader = true;
      if (!width || !height || payload[8] !== 8 || payload[9] !== 3)
        throw new Error('Replacement must be an 8-bit indexed PNG, not RGB or RGBA.');
      if (payload[10] || payload[11] || payload[12])
        throw new Error('Compressed, filtered, or interlaced PNG variants are not supported.');
    } else if (type === 'PLTE') {
  ...
  if (!sawHeader || !sawEnd || !palette || !compressed.length)
    throw new Error('PNG is missing required indexed-image chunks.');
  ...
  for (let index = 0; index < pixels.length; index += 1) {
    ...
    if (value !== 0 && value !== 255)
      throw new Error('Replacement PNG contains partial transparency; sprites require binary transparency.');
    alpha[index] = value;
  }
  const fullPalette = new Uint8Array(768);
  fullPalette.set(palette);
